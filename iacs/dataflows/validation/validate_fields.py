...
def validated_components(
    ...
    This is a simplified version of ``validated_data`` in ``validate_registry`` that
    uses the raw ``field`` table rather than the inheritance-resolved ``derived_field``.
    ...
    for ctype, table in components.items():
        ...
        # Collect constraint violations as lazy ibis sub-queries
        for fname, fschema in schema.items():
            if fname not in existing:
                continue
