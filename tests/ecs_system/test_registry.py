...
class TestRegistryEntityIdForAlias:
    """Tests for entity_id_for_alias(), which resolves an alias to its existing entity_id."""
    ...
    def sample_registry(self):
        ...
        conn.create_table(
            "entity_id",
            {"value": ["e_door", "e_expanse"], "alias": ["door", "expanse"],
             "path": ["setting.yaml:door", "setting.yaml:expanse"],
             "entity_key": ["door", "expanse"], "filepath": ["setting.yaml", "setting.yaml"]},
        )
        return Registry(conn, {"entity_id": conn.table("entity_id")})

    def test_returns_entity_id_for_known_alias(self, sample_registry):
        assert sample_registry.entity_id_for_alias("door") == "e_door"

    def test_raises_keyerror_for_unknown_alias(self, sample_registry):
        with pytest.raises(KeyError):
            sample_registry.entity_id_for_alias("nonexistent")


class TestRegistrySetTag:
    """Tests for set_tag(), which writes a bare tag onto entities looked up by alias."""
    ...
    def sample_registry(self):
        ...
        conn.create_table(
            "entity_id",
            {"value": ["e_door", "e_expanse", "e_player"], "alias": ["door", "expanse", "player"],
             "path": ["setting.yaml:door", "setting.yaml:expanse", "setting.yaml:player"],
             "entity_key": ["door", "expanse", "player"],
             "filepath": ["setting.yaml", "setting.yaml", "setting.yaml"]},
        )
        return Registry(conn, {"entity_id": conn.table("entity_id")})

    def test_tags_entities_by_their_existing_entity_id(self, sample_registry):
        """The written tag lands on the alias's already-registered entity_id,
        not a new one minted from some other source (see the story-simulator
        bug this fixes: a tag written under a different source than the
        entity's own file never resolved back to it)."""
        sample_registry.set_tag("active", {"door", "expanse"})

        df = sample_registry.view("active").execute()
        assert set(df["entity_id.alias"]) == {"door", "expanse"}
        assert set(df["entity_id"]) == {"e_door", "e_expanse"}

    def test_overwrites_previous_tags_wholesale(self, sample_registry):
        """A later call with a different alias set drops the tag from
        whichever entities left it, rather than accumulating."""
        sample_registry.set_tag("active", {"door", "expanse"})
        sample_registry.set_tag("active", {"player"})

        df = sample_registry.view("active").execute()
        assert set(df["entity_id.alias"]) == {"player"}

    def test_empty_aliases_clears_the_tag(self, sample_registry):
        sample_registry.set_tag("active", {"door"})
        sample_registry.set_tag("active", set())

        df = sample_registry.view("active").execute()
        assert df.empty

    def test_unknown_alias_raises_keyerror(self, sample_registry):
        with pytest.raises(KeyError):
            sample_registry.set_tag("active", {"nonexistent"})
