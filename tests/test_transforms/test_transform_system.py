...
from tests.test_transforms import test_transform_dataflow, test_transform_dataflow_b
...
class TestTransformSystemConstruction:
    def test_can_create_with_registry_and_dataflows(self):
        ...
        ts = TransformSystem(registry, [test_transform_dataflow])
        assert ts is not None

    def test_accepts_multiple_dataflow_modules(self):
        registry = _sample_registry()
        ts = TransformSystem(registry, [test_transform_dataflow, test_transform_dataflow_b])
        assert ts is not None
